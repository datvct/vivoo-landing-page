...
const menus = [
  ...
  },
  {
    ...
    submenu: ["Case Studies", "Success Stories", "Testimonials", "Use Cases"],
  },
  {
    label: "Resources",
    submenu: [
  ...
  {
    label: "Partners",
    submenu: ["Partner Program", "Find a Partner", "Become a Partner"],
  },
];
    ...
    <nav className="hidden gap-8 lg:flex">
      {menus.map((item) => (
        ...
        >
          ...
          {/* Dropdown Menu */}
          <div
            className={`absolute left-0 mt-0 w-56 bg-white rounded-lg shadow-lg border border-gray-100 transform origin-top transition-all duration-300 ${
          ...
          <div
            className={`absolute bottom-0 left-0 h-0.5 bg-blue-600 transition-all duration-300 ${
              hoveredMenu === item.label ? "w-full" : "w-0"
            }`}
          />
        </div>
      ))}
