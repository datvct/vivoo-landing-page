...
export default function ServiceManagementPage() {
  ...
  // Fetch Services Query Hook
  const { data, isLoading } = useAdminServicesQuery({
    page,
    limit,
  ...
  function openEdit(record: Service) {
    router.push(`/admin/services/${record.id}/edit`);
  }
  ...
  const columns: ColumnsType<Service> = [
    ...
    },
    {
      ...
      render: (title: string, record: Service) => (
        <div>
        ...
        <div className="flex flex-col gap-0.5 text-xs">
          {record.primaryActionHref && (
            <span className="text-slate-500">
              Primary: <a href={record.primaryActionHref} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">Link</a>
            </span>
          )}
          {record.secondaryActionHref && (
            <span className="text-slate-500">
              Secondary: <a href={record.secondaryActionHref} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">Link</a>
            </span>
          )}
          {!record.primaryActionHref && !record.secondaryActionHref && <span className="text-slate-400">-</span>}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record: Service) => (
        <AdminActionMenu
          ...
          onTranslate={() => router.push(`/admin/services/create?translateFromId=${record.id}`)}
        />
      ),
    },
  ];
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Manage your services, thumbnails, custom CTAs, SEO metadata, and rich HTML contents.
            </p>
          </div>
          <Button
            type="primary"
