...
import type { PhoneRegion } from "../../../types/support-detail-types";
...
export default function SupportPhoneSection({
  title,
  description,
  regions,
}: SupportPhoneSectionProps) {
  const [
    activeRegion,
    setActiveRegion,
  ] = useState(0);
            ...
            {region.items.map(
              (item) => (
                <div key={item.country}>
                  ...
                  <div className="mt-2 text-xs font-medium tracking-[0.2em] text-black/45 uppercase">
                    Phone
                  </div>
                  <a
