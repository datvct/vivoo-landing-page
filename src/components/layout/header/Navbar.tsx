...
export default function Navbar({ menus = [] }: NavbarProps) {
  ...
  const activeMenus = menus && menus.length > 0 ? menus : [];
        ...
        >
          ...
          >
            ...
            {item.submenu && item.submenu.length > 0 && (
              <ChevronDown
                size={16}
                className={`transition-transform duration-300 ${hoveredMenu ===
                  item.label
                  ? "rotate-180"
                  : ""
                  }`}
          ...
          {item.submenu && item.submenu.length > 0 && (
            <div
              className={`absolute left-0 mt-0 w-56 origin-top transform rounded-lg border border-gray-100 bg-white shadow-lg transition-all duration-300 ${hoveredMenu === item.label
          ...
          <div
            className={`absolute bottom-0 left-0 h-0.5 bg-blue-600 transition-all duration-300 ${hoveredMenu === item.label
              ? "w-full"
              : "w-0"
              }`}
          />
